...
from utils.data_loading import SatteliteTestDataset
...
def main():
    ...
    sat_full = sample["sat_full"]
    gt_full = sample["gt_full"]
    valid_mask_full = sample["valid_mask_full"]

    tile_index = 399
    plt.imshow(sat[0, tile_index].permute(1, 2, 0))
    plt.imshow(
        gt[0, tile_index].permute(1, 2, 0) * valid_mask[0, tile_index].permute(1, 2, 0),
        alpha=0.3,
    )
    plt.title("Sattelite Patch with Segmentation Mask")
    plt.show()

    plt.imshow(sat_full[0].permute(1, 2, 0))
    plt.imshow(
        gt_full[0].permute(1, 2, 0) * valid_mask_full[0].permute(1, 2, 0),
        alpha=0.3,
    )
    plt.title("Sattelite Kaartblad with Segmentation Mask")
    plt.show()
